...
from abiflow_skill_lib import recommend_wannier_setup  # noqa: E402
...
def test_isolated_insulator_recommendation_is_concrete():
    ...
    assert rec["window_recommendation"]["strategy"] == "omit"
    assert rec["validation_checks"]
    assert rec["first_revision_actions"]
...
def test_entangled_metal_recommendation_contains_window_logic():
    ...
    assert rec["window_recommendation"]["strategy"] in {"moderate", "broad"}
