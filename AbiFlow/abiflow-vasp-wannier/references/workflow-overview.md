...
- For remote or background execution, prefer the simplest reproducible launch form and always keep a dedicated log file rather than building fragile compound shell commands.
...
- Do not map `num_wann` directly from the number of projection lines; count equivalent atoms, spin channels, and any spinor doubling explicitly.
- Decide whether the case is isolated-band or entangled-band before discussing windows.
- Before increasing `num_wann`, count how many KS states the chosen outer window contains at every `k` point on the actual uniform mesh.
...
- Do not use spread alone as the pass/fail metric; check missing bands in the target window, the 95% interpolation error, and the worst local mismatch or path segment.
...
- If disentanglement is slow but still monotonic and error-free, let it run to convergence or the iteration limit before rejecting the setup.
