...
4. Keep the VASP inputs lean, do not add structural relaxation by default, use `PREC = Normal`, use `EDIFF = 1E-6`, use `LREAL = .FALSE.`, keep SCF at `ISYM = 2`, and switch SOC-enabled non-SCF steps to `ISYM = -1`.
