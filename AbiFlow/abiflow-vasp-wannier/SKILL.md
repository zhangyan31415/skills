...
- In parallel VASP runs, `NBANDS` may be increased automatically to satisfy the parallel layout. For any later band counting or post-processing, use the actual value reported in `OUTCAR` and the generated interface files, not `INCAR` alone.
- Do not assume a hand-written `NBANDS` equals the final interface-side `num_bands`; use the actual `wannier90.win` and `wannier90.eig` generated by VASP as the source of truth for later windowing, band counting, and diagnostics.
...
- In a standard non-SCF band step, messages such as `WARNING: stress and forces are not correct` are usually expected and should not be treated as a failure by themselves.
...
- Stopping at “spread is lower” when the interpolated bands are worse; a slightly larger spread can still correspond to a more correct target subspace once `exclude_bands` and frozen/outer windows are chosen properly.
- Judging spread quality from `Omega Total` alone when many Wannier functions are present; inspect the final per-WF spread distribution as well.
- Treating poor fits like a windows-only problem when the actual issue is an incomplete projection basis.
- Tuning windows independently of the target subspace; unstable windows often mean the target subspace itself is not yet defined sharply enough.
